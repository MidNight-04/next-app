  ...
  return (
    <AsideContainer>
      ...
      <div className="bg-white p-6 rounded-3xl shadow-xl">
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-4 -md:grid-cols-1 mb-4">
          {[
            { name: "name", label: "Name", type: "text", placeholder: "Enter Name" },
            { name: "email", label: "Email", type: "email", placeholder: "Enter Email" },
            { name: "phone", label: "Phone", type: "number", placeholder: "Enter Phone" },
            { name: "address", label: "Address", type: "text", placeholder: "Enter Address" },
          ].map((field) => (
            <div key={field.name} className="flex flex-col gap-3">
              <label htmlFor={field.name} className="text-lg font-semibold text-secondary">
                {field.label}
              </label>
              <input
                {...register(field.name)}
