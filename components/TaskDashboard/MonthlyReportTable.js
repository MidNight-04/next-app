...
const MonthlyReportTable = ({ isFetched, groupedByMonth, activeTab }) => {
  return (
    <>
      <div className="w-full overflow-x-auto bg-secondary font-semibold rounded-2xl">
          ...
          <tbody>
            {isFetched &&
              ...
              groupedByMonth
                ...
                //     const filteredTasks = item.obj.filter(task =>
                //       selectedTeam.includes(task.issueMember?._id)
                //     );
                //     return {
                //       ...item,
                //       obj: filteredTasks,
                ...
                .map((item, index) => {
                  const percent =
                    (item.obj.filter(task => task.status === 'Complete')
                      .length /
                      item.obj.length) *
                    100;
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <div className="hidden group-hover:flex -md:flex mt-2 flex-row gap-2 justify-between w-full px-8">
                            <span className="text-green-500">
                              {completedInTime} (
                              {((completedInTime / totalTasks) * 100).toFixed(
                                0
                              )}
                              %)
                            </span>
                            <span className="text-red-500">
                              {completedDelayed} (
                              {((completedDelayed / totalTasks) * 100).toFixed(
                                0
                              )}
                              %)
                            </span>
        ...
        {groupedByMonth?.filter(item => item.status === activeTab)?.length >=
          10 && (
